...
public class FileComparator {
    ...
    public FileComparator(List<List<String>> duplicates) {
    ...
    // Метод для побайтного сравнения массива файлов
    public void compareFilesRecursively(File[] files) throws IOException {
        if (files == null || files.length < 2) {
            return; // Если массив пуст или содержит менее двух файлов, выходим
        }

        List<File> remainingFiles = new ArrayList<>();
        List<String> currentGroup = new ArrayList<>();
        currentGroup.add(files[0].getAbsolutePath());

        for (int i = 1; i < files.length; i++) {
            if (areFilesEqual(files[0].toPath(), files[i].toPath())) {
                currentGroup.add(files[i].getAbsolutePath());
            } else {
                remainingFiles.add(files[i]);
            }
        }
        ...
        if (!remainingFiles.isEmpty()) {
            // Рекурсивно вызываем метод для оставшихся файлов
            compareFilesRecursively(remainingFiles.toArray(new File[0]));
        }
    }
